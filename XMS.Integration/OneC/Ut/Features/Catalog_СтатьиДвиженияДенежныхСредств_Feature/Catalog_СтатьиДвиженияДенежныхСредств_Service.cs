    ...
    : BaseService, ICatalog_СтатьиДвиженияДенежныхСредств_Service
{
    ...
    public async Task<ServiceResult> HandleEventOneC(CatalogEvent catalogEvent, CancellationToken ct = default)
    {
        logger.LogDebug("{Source} - Start {@message}", nameof(HandleEventOneC), catalogEvent);

        var fetchedItem = await FetchByRefKeyAsync(catalogEvent.Ref_Key, ct);

        if (fetchedItem is null)
        {
            logger.LogError("{Source} - Failed to feath {@message}", nameof(HandleEventOneC), catalogEvent);
        ...
        await dbContext.Set<Catalog_СтатьиДвиженияДенежныхСредств>()
            .Where(e => e.Ref_Key == catalogEvent.Ref_Key)
        ...
        logger.LogDebug("{Source} - Ok {@message} {@fetchedItem}", nameof(HandleEventOneC), catalogEvent, fetchedItem);
