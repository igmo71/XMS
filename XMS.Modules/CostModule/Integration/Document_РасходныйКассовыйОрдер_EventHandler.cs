...
internal class Document_РасходныйКассовыйОрдер_EventHandler(IDbContextFactoryProxy dbFactory) : IDocument_РасходныйКассовыйОрдер_EventHandler
{
    public async Task HandleReceivedAsync(Document_РасходныйКассовыйОрдер_Dto dto, CancellationToken ct = default)
    {
    ...
    public async Task HandleDeletedAsync(Document_РасходныйКассовыйОрдер_Dto dto, CancellationToken ct = default)
    {
        using var dbContext = dbFactory.CreateDbContext();
